...
export default function Dashboard(){
    ...
    setTimeout(() => {
      ...
      spotify.getMyRecentlyPlayedTracks(function (err, data) {
        if (err) console.error(err);
          ...
          console.log(
            "Url image ",
            data.items[0].track.album.images.length !== 0
  ...
  console.log("FONCTIONNE", recents.length !== 0 ? recents.items[0].track.name : "Artiste");
  console.log("url image", recents.length !== 0 ? recents.items[0].track.album.images[0].url : "");
